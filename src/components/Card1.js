    ...
    const verifyPayment = async (e) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await axios.post('/admin/workshop-payment-success', {
    ...
    const declinePayment = async (e) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                await axios.post('/admin/workshop-payment-failure', {
                    transactionId: data.transactionId
                }, {
                    headers: {
                        "token": localStorage.getItem("token")
                ...
                <div className="card w-full bg-neutral text-neutral-content m-2">
                    <div className="card-body items-center text-center">
                        <figure className="px-10 pt-10" >
                            <img src={`${process.env.REACT_APP_API_BASE_URL}/images/${data.screenshot}`} width="100px" onClick={() => document.getElementById(`modal_${data.transactionId}`).showModal()} alt="Screenshot" className="rounded-xl" />
                        </figure>
                        ...
                        <div className="card-actions justify-end">
                            <button className="btn btn-primary" onClick={verifyPayment}>Accept</button>
                            <button className="btn btn-ghost" onClick={declinePayment}>Deny</button>
                        </div>
                    </div>
                </div>
                <dialog id={`modal_${data.transactionId}`} className="modal">
                    <div className="modal-box">
                        <img src={`${process.env.REACT_APP_API_BASE_URL}/images/${data.screenshot}`} alt="Screenshot" className="rounded-xl" />
